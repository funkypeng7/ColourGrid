...
void CheckDirection()
{
    ...
    {
        std::vector<float> deltaVector = {prevRawCoords[1] - initialPress[1], prevRawCoords[2] - initialPress[2]};

        // Up
        if(deltaVector[1] > 100 && (lastDirectionTraveled != 2 || snakeLength == 1))
            direction = 0;
        // Right
        else if(deltaVector[0] > 100 && (lastDirectionTraveled != 3 || snakeLength == 1))
            direction = 1;
        // Down
        else if(deltaVector[1] < -100 && (lastDirectionTraveled != 0 || snakeLength == 1))
            direction = 2;
        // Left
        else if(deltaVector[0] < -100 && (lastDirectionTraveled != 1 || snakeLength == 1))
            direction = 3;
